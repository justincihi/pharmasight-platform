...
class AutonomousResearchEngine:
    ...
    def _search_pubmed(self, query: str, max_results: Optional[int] = None) -> List[Dict]:
        """
    ...
    def _save_session_log(self, summary: Dict):
        ...
        log_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'research_logs')
        os.makedirs(log_dir, exist_ok=True)
        log_path = os.path.join(log_dir, f"research_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
